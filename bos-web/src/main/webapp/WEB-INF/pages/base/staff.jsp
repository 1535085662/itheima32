...
<script type="text/javascript">
	...
	function doView(){
		alert("查看...");
		$('#inquireStaffWindow').window("open");
	}
		...
		if (rows.length == 0) {
			...
			$.messager.confirm("删除确认","你确定要删除选中的取派员吗？",function(r){
				if(r){
					...
					location.href = "staffAction_deleteBatch.action?ids="+ids;//发送get请求
				}
			});
		}
	}
	...
	$(function(){
		...
		$('#addStaffWindow').window({
	        ...
	        closed: true,//关闭
	        height: 400,
	        resizable:false//窗口大小是否可跳转
	    });
		
		// 查看取派员窗口
		$('#inquireStaffWindow').window({
	        title: '查看取派员',
	        width: 400,
	        modal: true,//遮罩层
	        shadow: true,//阴影效果
	...
	function doDblClickRow(rowIndex, rowData){
		...
		$("#editStaffForm").form("load",rowData);//回显数据
	}
</script>	
</head>
<body class="easyui-layout" style="visibility:hidden;">
	...
	<!-- 查看取派员窗口开始 -->
	<div class="easyui-window" title="对收派员进行添加或者修改" id="inquireStaffWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		<div region="north" style="height:31px;overflow:hidden;" split="false" border="false" >
			<div class="datagrid-toolbar">
				<a id="save" icon="icon-search" href="#" class="easyui-linkbutton" plain="true" >查詢</a>
			</div>
		</div>
		
		<div region="center" style="overflow:auto;padding:5px;" border="false">
			<form id="inquireStaffForm" action="staffAction_inquire.action" method="post">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
							...
							<script type="text/javascript">
								$(function(){
									$("#save").click(function(){
										...
										var v = $("#inquireStaffForm").form("validate");
										if (v) {
											$("#inquireStaffForm").submit();
										}
									});
